...
const EDGE_OVERLAY_SELECTOR = '.browser-chrome-edge-overlay';
...
const getColorAtViewportEdge = (edge) => {
  ...
  const overlay = [...document.querySelectorAll(EDGE_OVERLAY_SELECTOR)]
    .find((section) => {
      const rect = section.getBoundingClientRect();
      const style = window.getComputedStyle(section);
      const edgeTolerance = edge <= 12 ? 12 : 3;
      return style.display !== 'none'
        && style.visibility !== 'hidden'
        && Number.parseFloat(style.opacity) > 0
        && rect.left <= x
        && rect.right >= x
        && rect.top <= edge + edgeTolerance
        && rect.bottom >= edge - edgeTolerance;
    });

  if (overlay) return getSectionColor(overlay);
  ...
  return getSectionColor(candidates[0]);
};
